...
| `phase_01_sla1_telemetry.feature` | 1 | FR-1–6 |
| `phase_02a_shore_race_prep.feature` | 2A | FR-133–149, FR-173–180 |
| `phase_02b_graph_import.feature` | 2B | FR-102–104 |
| `phase_02c_grib_polars_ais.feature` | 2C | FR-15–26 |
...
| `phase_04_cicd.feature` | 4 | FR-90–101, FR-105 |
...
# Phase 0 only (implemented today)
pytest tests/bdd/steps/test_phase_00_foundation.py -v

# All phases — pending scenarios are skipped
pytest tests/bdd/steps/ -v
...
When implementing a phase, replace `pytest.skip(...)` in the matching `tests/bdd/steps/test_phase_*.py` with real assertions (HTTP health checks, Neo4j queries, file artifacts, etc.). Keep scenario text in `.feature` files stable — change step bodies only.
